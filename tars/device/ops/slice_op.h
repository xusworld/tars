...
class SliceOp : public Operator<rtype, dtype> {
 public:
  // C++ type traits trick
  ...
  virtual Status run(const TensorShape &shape) {
    // SliceOp with a single input and a single output.
    CHECK(inputs_.size() == 1);
    CHECK(outputs_.size() == 1);

    this->shape_ = shape;

    // SliceOp's input and output should share the same buffer.
    return Status::OK();
  }
