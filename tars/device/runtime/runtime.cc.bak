...
MaceStatus Runtime::AfterRun() { return MaceStatus::MACE_SUCCESS; }
...
bool Runtime::CanReuseBuffer(const Buffer *buffer,
  ...
  auto size_bytes = std::accumulate(shape.begin(), shape.end(), 1,
...
MemoryType Runtime::GetBaseMemoryType() { return MemoryType::CPU_BUFFER; }

MemoryType Runtime::GetUsedMemoryType() { return GetBaseMemoryType(); }

utils::ThreadPool &Runtime::thread_pool() { return *thread_pool_; }
...
std::vector<index_t> Runtime::ComputeBufDimFromTensorDim(
  ...
  MACE_UNUSED(content_param);
  auto size =
      std::accumulate(dims.begin(), dims.end(), 1, std::multiplies<index_t>());
  if (size == 0) {
...
MaceStatus Runtime::AllocateBufferForTensor(Tensor *tensor,
                                            BufRentType rent_type,
                                            Buffer *slice_parent,
                                            index_t offset_bytes) {
...
void Runtime::SetBufferToTensor(std::unique_ptr<Buffer> buffer,
                                Tensor *tensor) {
