...
class ConfigFile {
 ...
 public:
  ...
  ConfigFile() = default;
  template <class T>
  T Read(const std::string& in_key)
      const;  //!< Search for key and read value or optional default value, call
              //!< as read<T>
  template <class T>
...
template <class T>
bool ConfigFile::ReadInto(T& var, const std::string& key,
                          const T& value) const {
  // Get the value corresponding to key and store in var
  // Return true if key is found
...
std::vector<int> stringToDims(const std::string& str,
...
std::vector<std::vector<int>> splitDims(const int size,
...
std::ostream& operator<<(std::ostream& os, const ConfigFile& cf) {
  ...
  for (ConfigFile::mapci p = cf.m_Contents.begin(); p != cf.m_Contents.end();
       ++p) {
  ...
  while (is || nextline.length() > 0) {
    ...
    if (delimPos < std::string::npos) {
      ...
      while (!terminate && is) {
        ...
        if (nextline.find(delim) != std::string::npos) continue;
...
void ConfigFile::ReadFile(std::string filename, std::string delimiter,
                          std::string comment) {
