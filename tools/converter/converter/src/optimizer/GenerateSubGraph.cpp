...
static void _mergeSubGraph(std::shared_ptr<ClusterNode> node) {
  ...
  for (auto c : children) {
    ...
    node->children.insert(node->children.end(), c->children.begin(),
                          c->children.end());
  }
}
...
static bool _isControlOp(const OpT* op) {
  std::set<std::string> controlOps{"Merge", "Switch", "LoopCond",
                                   "Enter", "Exit",   "NextIteration"};
  return op->type == OpType_Extra &&
         controlOps.find(op->main.AsExtra()->type) != controlOps.end();
}

std::vector<std::unique_ptr<OpT>> _makeCond(
    std::shared_ptr<ClusterNode> cNode, ace::NetT* netT,
    const std::map<std::string, int>& originTensorIndexes) {
  ...
  if (condTensorIndexes.size() > 1) {
    ...
    while (needCheck) {
      ...
      for (int i = 0; i < nodes.size(); ++i) {
        if ((!needCheck) && nodes[i]->type == OpType_Extra &&
            nodes[i]->main.AsExtra()->type == "Switch") {
          if (nodes[i]->inputIndexes[1] != condTensorIndex) {
            // Once Time remove only one switch
            for (auto output : nodes[i]->outputIndexes) {
              replaceTensor.insert(
                  std::make_pair(output, nodes[i]->inputIndexes[0]));
            }
            needCheck = true;
            continue;
          }
  ...
  for (int i = 0; i < cNode->nodes.size(); ++i) {
    ...
    for (auto index : op->outputIndexes) {
      if (tensorMask[index] > 0) {
        assert(opMask[i] <= 0 || opMask[i] == tensorMask[index]);
        opMask[i] = tensorMask[index];
        valid = true;
      }
    }
  ...
  while (needCheck) {
    ...
    auto nodes = std::move(cNode->nodes);
    for (int i = 0; i < nodes.size(); ++i) {
      if (nodes[i]->type == OpType_Extra &&
          nodes[i]->main.AsExtra()->type == "Switch" && (!needCheck)) {
        // Once Time remove only one switch
        for (auto output : nodes[i]->outputIndexes) {
          replaceTensor.insert(
              std::make_pair(output, nodes[i]->inputIndexes[0]));
  ...
  for (auto& node : cNode->nodes) {
    ...
    Express::Program::createUnit(varMap, inputIndexes, cNode->nodes, node.get(),
                                 netT, invalidSet, extraInputIndexes);
  }
  ...
  for (int i = 0; i < leftOutputs.size(); ++i) {
    ...
    condOp->main.AsIfParam()->aliases_outputs.emplace_back(
        std::move(outputPari));
  }
...
std::vector<std::unique_ptr<OpT>> _makeWhile(
    std::shared_ptr<ClusterNode> cNode, ace::NetT* netT,
    const std::map<std::string, int>& originTensorIndexes) {
  std::vector<std::unique_ptr<OpT>> res;
  ...
  std::set<std::string> bodyOutputNames;
  {
    ...
    for (auto& op : childs) {
      if (op->type == OpType_Extra && op->main.AsExtra()->type == "Merge") {
        int updateFromIdx = op->inputIndexes[1],
            updateToIdx = op->inputIndexes[0];
        // if tensor_x is at outside of loop and used by two op, and these two
        // op are all update data, so need copy tensor_x to tensor_x_copy.
  ...
  for (auto& op : cNode->nodes) {
    ...
    if (op->main.AsExtra()->type == "Exit") {
      ...
      whileParam->aliases_outputs.emplace_back(
          netT->tensorName[op->inputIndexes[0]]);
  ...
  }
  {
    ...
    assert(condGraph->outputs.size() > 0);
  }
  {
    ...
    for (auto& op : bodyNet->oplists) {
      ...
      for (auto o : op->outputIndexes) {
        if (bodyOutputNames.find(bodyNet->tensorName[o]) !=
            bodyOutputNames.end()) {
      ...
      for (auto& s : whileParam->aliases_updates) {
        ...
        for (int i = 0; i < subGraph->nodes.size(); ++i) {
          ...
          if (subGraph->tensors[op->outputIndexes[0]] == second) {
            ...
            subGraph->nodes[i]->main.AsInput()->dims =
                opPtr->main.AsBlob()->dims;
            subGraph->nodes[i]->main.AsInput()->dtype =
                opPtr->main.AsBlob()->dataType;
            subGraph->nodes[i]->main.AsInput()->dformat =
                opPtr->main.AsBlob()->dataFormat;
...
static std::vector<std::unique_ptr<OpT>> _makeSubGraph(
    std::shared_ptr<ClusterNode> cNode, ace::NetT* netT,
...
int GenerateSubGraph(std::unique_ptr<ace::NetT>& netT) {
  ...
  //   LOG(INFO)
  //   << "The modle has control flow, please use  ace::Module to run it\n";
