...
#include "Program.hpp"
#include "express/ExprCreator.hpp"
...
void Program::createUnit(std::map<int, VARP>& varMap,
      ...
      for (int j = 0; j < oplists.size(); ++j) {
        for (auto outputIndex : oplists[j]->outputIndexes) {
          if (outputIndex == input) {
            createUnit(varMap, inputIndexes, oplists, oplists[j].get(), net,
                       invalidSet, extraInputIndexes);
          }
        }
      }
      if (varMap.find(input) == varMap.end()) {
        extraInputIndexes.insert(input);
        //                MNN_PRINT("Don't find input %d - %s for %s, turn to
...
std::shared_ptr<Program> Program::create(const ace::NetT* net,
                                         bool supportExtra) {
  ...
  for (int index = 0; index < net->oplists.size(); ++index) {
    std::set<OpT*> invalidSet;
    createUnit(varMap, inputIndexes, net->oplists, net->oplists[index].get(),
               net, invalidSet, extraInputIndexes);
  }
